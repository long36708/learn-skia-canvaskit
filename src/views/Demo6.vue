...
function getTypeFace() {
  // try {
  //   const data = localStorage.getItem("typeface");
...
function DrawingExample(CanvasKit, gpu, robotoData) {
  ...
  // HACK
  window.typeface = roboto;
  localStorage.setItem("typeface", roboto);
...
function InkExample(CanvasKit, gpu) {
  ...
  const interact = (e) => {
    ...
    if (hold) {
      ...
      paint = paint.copy();
      paint.setColor(CanvasKit.Color(Math.random() * 255, Math.random() * 255, Math.random() * 255, Math.random() + 0.2));
      paints.push(paint);
...
function CanvasAPI1(CanvasKit) {
  const skcanvas = CanvasKit.MakeCanvas(300, 300);
  const realCanvas = document.getElementById("api1_c");

  const skPromise = fetch(`${cdn}test.png`)
  ...
  const realPromise = fetch(`${cdn}test.png`)
  ...
  const realFontLoaded = new FontFace("Bungee", "url(/tests/assets/Bungee-Regular.ttf)", {
    family: "Bungee",
    style: "normal",
    weight: "400",
  }).load().then((font) => {
  ...
  const skFontLoaded = fetch("/tests/assets/Bungee-Regular.ttf").then(
...
function CanvasAPI2(CanvasKit) {
  const skcanvas = CanvasKit.MakeCanvas(300, 300);
  const realCanvas = document.getElementById("api2_c");
...
function CanvasAPI3(CanvasKit) {
  const skcanvas = CanvasKit.MakeCanvas(300, 300);
  const realCanvas = document.getElementById("api3_c");
  realCanvas.width = 300;
  realCanvas.height = 300;
  ...
  for (const canvas of [skcanvas, realCanvas]) {
    ...
    ctx.save();
    ctx.setTransform(2, 0, -0.5, 2.5, -40, 120);
...
function CanvasAPI4(CanvasKit) {
  const skcanvas = CanvasKit.MakeCanvas(300, 300);
  const realCanvas = document.getElementById("api4_c");
  realCanvas.width = 300;
  realCanvas.height = 300;
...
function CanvasAPI5(CanvasKit) {
  ...
  const realCanvas = document.getElementById("api5_c");
...
function CanvasAPI6(CanvasKit) {
  const skcanvas = CanvasKit.MakeCanvas(600, 600);
  const realCanvas = document.getElementById("api6_c");
  realCanvas.width = 600;
  realCanvas.height = 600;
...
function CanvasAPI7(CanvasKit) {
  const skcanvas = CanvasKit.MakeCanvas(300, 300);
  const realCanvas = document.getElementById("api7_c");
  ...
  Promise.all([realPromise, skPromise]).then(() => {
    for (const canvas of [skcanvas, realCanvas]) {
      ...
      ctx.fillStyle = pattern;
      pattern.setTransform({ a: 1, b: -0.1, c: 0.1, d: 0.5, e: 1800, f: 800 });
...
function CanvasAPI8(CanvasKit) {
  const skcanvas = CanvasKit.MakeCanvas(300, 300);
  const realCanvas = document.getElementById("api8_c");
  ...
  const pts = [[3, 3], [4, 4], [5, 5], [10, 10], [8, 10], [6, 10], [6.5, 9], [15, 10], [17, 10], [17, 11], [24, 24], [25, 25], [26, 26], [27, 27]];
...
function VertexAPI1(CanvasKit, gpu) {
  ...
  const colors = [CanvasKit.RED, CanvasKit.BLUE, CanvasKit.YELLOW, CanvasKit.CYAN];
  let vertices = CanvasKit.MakeVertices(CanvasKit.VertexMode.TriangleFan, points, null, colors, false /* isVolatile */);
  ...
  vertices = CanvasKit.MakeVertices(CanvasKit.VertexMode.TriangleFan, points, texs, colors);

  const shader = CanvasKit.Shader.MakeLinearGradient([0, 0], [250, 0], colors, null, CanvasKit.TileMode.Clamp);
...
function GradientAPI1(CanvasKit, gpu) {
  ...
  const pos = [0, 0.7, 1.0];
  const transform = [2, 0, 0, 0, 2, 0, 0, 0, 1];
  const shader = CanvasKit.Shader.MakeRadialGradient([150, 150], 130, colors, pos, CanvasKit.TileMode.Mirror, transform);

  paint.setShader(shader);
  const textFont = new CanvasKit.Font(getTypeFace(), 75);
...
function TextOnPathAPI1(CanvasKit, gpu) {
  ...
  const font = new CanvasKit.Font(getTypeFace(), 24);
...
function DrawGlyphsAPI1(CanvasKit, gpu) {
  ...
  const canvas = surface.getCanvas();
  const paint = new CanvasKit.Paint();
  const font = new CanvasKit.Font(getTypeFace(), 16);
  paint.setAntiAlias(true);
...
function AtlasAPI1(CanvasKit, gpu, imgData) {
  ...
  function drawFrame(canvas) {
    canvas.clear(CanvasKit.WHITE);
    i++;
    const scale = 0.5 + Math.sin(i / 40) / 4;
    ...
    canvas.drawAtlas(img, srcs, dsts, paint, CanvasKit.BlendMode.Plus, colors, { filter: CanvasKit.FilterMode.Nearest });
    surface.requestAnimationFrame(drawFrame);
  }
...
async function startRendering() {
  ...
  Promise.all([ready, loadRoboto]).then((results) => {
    DrawingExample(...results.flat());
  });
  Promise.all([ready, loadTestImage]).then((results) => {
    AtlasAPI1(...results.flat());
  });
...
<template>
  <div>
    <h1>Hello Skia CanvasKit on Vite Demo6 !</h1>

    <h2> Drop in replacement for HTML Canvas (e.g. node.js)</h2>
    <img id="api1" width="300" height="300">
    <canvas id="api1_c" width="300" height="300" />
    <img id="api2" width="300" height="300">
    <canvas id="api2_c" width="300" height="300" />
    <img id="api3" width="300" height="300">
    <canvas id="api3_c" width="300" height="300" />
    <img id="api4" width="300" height="300">
    <canvas id="api4_c" width="300" height="300" />
    <img id="api5" width="300" height="300">
    <canvas id="api5_c" width="300" height="300" />
    <img id="api6" width="300" height="300">
    <canvas id="api6_c" width="300" height="300" />
    <img id="api7" width="300" height="300">
    <canvas id="api7_c" width="300" height="300" />
    <img id="api8" width="300" height="300">
    <canvas id="api8_c" width="300" height="300" />

    <h2 id="title1">
      CanvasKit expands the functionality of a stock HTML canvas
    </h2>
    <canvas id="vertex1" width="300" height="300" />
    <canvas id="gradient1" width="300" height="300" />
    <canvas id="patheffect" width="300" height="300" />
    <canvas id="paths" width="300" height="300" />
    <canvas id="pathperson" width="300" height="300" />
    <canvas id="ink" width="300" height="300" />
    <canvas id="surfaces" width="300" height="300" />
    <canvas id="atlas" width="300" height="300" />
    <canvas id="decode" width="300" height="300" />

    <h2 id="title2">
      CanvasKit can allow for text measurement/placement (e.g. breaking, kerning)
    </h2>
    <canvas id="textonpath" width="300" height="300" />
    <canvas id="drawGlyphs" width="300" height="300" />

    <h2 id="title3">
      Interactive drawPatch
    </h2>
    <canvas id="interdrawpatch" width="512" height="512" />
  </div>
</template>
