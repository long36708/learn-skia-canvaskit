  ...
  const ed = {
    ...
    draw(canvas) {
      ...
      while (start < this._text.length) {
        ...
        LOG("New range: ", start, end, "from run", r.textRange.start, r.textRange.end, "style", s_start, s_end);
    ...
    find_style_index_and_prev_length(index) {
      let len = 0;
    ...
    applyStyleToRange(style, start, end) {
      if (start > end) {
        [start, end] = [end, start];
      }
      ASSERT(start >= 0 && end <= this._text.length);
