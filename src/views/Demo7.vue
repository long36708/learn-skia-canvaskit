...
function GlyphGame(canvas, robotoData) {
  ...
  const letters = Array.from({ length: text.length });
  for (let i = 0; i < text.length; i++) {
...
function ColorSupport(CanvasKit) {
  ...
  const red = CanvasKit.Color4f(1, 0, 0, 1);
  const paint = new CanvasKit.Paint();
...
async function startRendering() {
  ...
  Promise.all([ckLoaded, loadDog, loadMandrill]).then((values) => {
    ...
    const dogShader = dogImg.makeShaderCubic(
      ...
      1 / 3,
      1 / 3,
      CanvasKit.Matrix.scaled(quadrantSize / dogImg.width(), quadrantSize / dogImg.height()),
    );
    ...
    const getBlurrySpiralShader = (rad_scale) => {
      ...
      return offscreenSurface.makeImageSnapshot()
        .makeShaderCubic(CanvasKit.TileMode.Clamp, CanvasKit.TileMode.Clamp, 1 / 3, 1 / 3);
    };
...
<template>
  <div>
    <h1>Hello Skia CanvasKit on Vite Demo7!</h1>
    <h2> Skottie </h2>
    <canvas id="sk_legos" width="300" height="300" />
    <canvas id="sk_drinks" width="500" height="500" />
    <canvas id="sk_party" width="500" height="500" />
    <canvas id="sk_onboarding" width="500" height="500" />
    <canvas
      id="sk_animated_gif" width="500" height="500"
      title="This is an animated gif being animated in Skottie"
    />

    <h2> RT Shader </h2>
    <canvas id="rtshader" width="300" height="300" />
    <canvas id="rtshader2" width="300" height="300" />

    <h2> Paragraph </h2>
    <canvas id="para1" width="600" height="600" />
    <canvas id="para2" width="600" height="600" tabindex="-1" />
    <canvas id="para3" width="600" height="600" tabindex="-1" />
    ``
    <h2> CanvasKit can serialize/deserialize .skp files</h2>
    <canvas id="skp" width="500" height="500" />

    <h2> 3D perspective transformations </h2>
    <canvas id="glyphgame" width="500" height="500" />

    <h2> Support for extended color spaces </h2>
    <a href="chrome://flags/#force-color-profile">Force P3 profile</a>
    <canvas id="colorsupport" width="300" height="300" />
  </div>
</template>
